...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        #     along with the tgt_mask.  the output to scores per token      #
        ############################################################################
        # *****START OF YOUR CODE (DO NOTProject DELETE/MODIFY THIS LINE)*****
        captions = self.embedding(captions)
        captions = self.positional_encoding(captions)
        visual = self.visual_projection(features).unsqueeze(1)
        tgt_mask = torch.tril(torch.ones(T, T)).unsqueeze(0).to(self.device)
