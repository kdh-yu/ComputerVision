...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions, attn=False):
        """
        ...
        visual = self.visual_projection(features).unsqueeze(1)
        tgt_mask = torch.tril(torch.ones(T, T)).to(self.device)
        if attn:
            self.transformer.out = True
            decode, attnw = self.transformer(captions, visual, tgt_mask=tgt_mask)
        else:
            decode = self.transformer(captions, visual, tgt_mask=tgt_mask)
        scores = self.output(decode)
    ...
    def sample(self, features, max_length=30, return_attn=False):
        """
        ...
        with torch.no_grad():
            ...
            for t in range(max_length):
                if return_attn:
                    output_logits, attnw = self.forward(features, partial_caption, attn=True)
                    attns.append(attnw)
                else:
...
class TransformerDecoderLayer(nn.Module):
    ...
    """
    def __init__(self, input_dim, num_heads, dim_feedforward=2048, dropout=0.1, return_attn=False):
        """
    ...
    def forward(self, tgt, memory, tgt_mask=None):
        ...
        # encoder layer.
        if self.out:
            tgt2, attnw = self.multihead_attn(query=tgt, key=memory, value=memory, return_attn=True)
            attn.append(attnw)
        else:
...
class TransformerDecoder(nn.Module):
    def __init__(self, decoder_layer, num_layers, return_attn=False):
        super().__init__()
