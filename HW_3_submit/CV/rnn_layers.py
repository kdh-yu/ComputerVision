...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    dgate[:, 3*H:] = 1 - dgate[:, 3*H:]**2
    dnext_c_tanh = 1 - next_c_tanh**2
    
    dnext_c_prod = dnext_h*o*dnext_c_tanh + dnext_c
    dgate[:, :H] = dgate[:, :H] * dnext_c_prod * g
    dgate[:, H:2*H] = dgate[:, H:2*H] * dnext_c_prod * prev_c
    dgate[:, 2*H:3*H] = dgate[:, 2*H:3*H] * dnext_h * next_c_tanh
    dgate[:, 3*H:] = dgate[:, 3*H:] * dnext_c_prod * i
...
def lstm_backward(dh, cache):
    ...
    for t in range(T-1, -1, -1):
        dx_t, dh0, dc, dWx_t, dWh_t, db_t = lstm_step_backward(dh0+dh[:, t, :], dc, cache[t])
        dx[:, t] = dx_t
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
