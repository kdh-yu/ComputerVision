...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        mul = torch.pow(10_000, torch.arange(0, embed_dim, 2).float()/(-embed_dim))
        #print(mul.shape)
        #position = torch.arange(0, max_len, dtype=torch.float).unsqueeze(1)
        #div_term = torch.exp(torch.arange(0, embed_dim, 2).float() * (-math.log(10000.0) / embed_dim))
        #print(position.shape)
        pe[0, :, 0::2] = torch.sin(pos * mul)
        pe[0, :, 1::2] = torch.cos(pos * mul)
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0.1, return_attn=False):
        """
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        mhattention = torch.matmul(attention_weights, v)
        mhattention = mhattention.transpose(1, 2).contiguous().view(N, S, E)
        output = self.proj(mhattention)
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
